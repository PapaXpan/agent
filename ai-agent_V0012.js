(function () {
  ...
  // ── Config ──────────────────────────────────────────────────────────────
  var API_URL_DIRECT =
    "https://gko7i4ny8d.execute-api.ca-central-1.amazonaws.com/dev/agent";
  var API_URL_PROXY =
    "https://corsproxy.io/?" + encodeURIComponent(API_URL_DIRECT);
  var CLIENT_ID = "1070";
  ...
  var STREAM_URL =
    "https://sa43r36p33yo7zhxfthtkcoj2u0nglcb.lambda-url.ca-central-1.on.aws/";
  ...
  var COURSES = [
    ...
    {
      value: "Interpreting Electrical Wiring Schematics",
      label: "Electrical Wiring Schematics",
    },
    {
      value: "BEB Technician Safety and Familiarization",
      label: "BEB Technician Safety",
    },
    ...
    {
      value: "Hatch - Sales Foundations - Proposal Development",
      label: "Hatch Sales Foundations",
    },
  ];
  ...
  var CHIPS_DEFAULT = [
    ...
    {
      label: "Summarize key topics",
      msg: "Summarize the key topics of this course.",
    },
    {
      label: "Give me a quiz question",
      msg: "Give me a quiz question from this course.",
    },
    {
      label: "What should I learn first?",
      msg: "What should I learn first in this course?",
    },
  ];
  ...
  var CHIPS_SLIDE = [
    ...
    {
      label: "What is this slide about?",
      msg: "What is the current slide about?",
    },
    {
      label: "Quiz me on this slide",
      msg: "Give me a quiz question based on this slide.",
    },
    {
      label: "Explain this in simple terms",
      msg: "Explain the content of this slide in simple terms.",
    },
  ];
  ...
  function buildChipsHTML(chips) {
    return chips
      .map(function (chip) {
        return (
          '<button class="xact-chip" data-msg="' +
          chip.msg +
          '">' +
          chip.label +
          "</button>"
  ...
  overlay.innerHTML = [
    ...
    '<p id="xact-course-label">course: budgeting &nbsp;&middot;&nbsp; client: ' +
      CLIENT_ID +
      "</p>",
    "</div>",
    ...
    '<div id="xact-starters">',
    buildChipsHTML(CHIPS_DEFAULT),
    "</div>",
  ...
  // Always present in HTML5 output. Text format: "slide: About the Course"
  // Priority 2: Other known Storyline DOM selectors.
  // Priority 3: document.title as last resort.
  //
  function detectSlideTitle() {
    // Priority 1: #slide-label (most reliable — Storyline 360 native)
  ...
  //   1. Reads #slide-label from the DOM for the current slide title
  //   2. Cross-references the title against SLIDE_CONTENT for rich content
  //   3. Falls back to DOM text scraping if no SLIDE_CONTENT match
  //   4. Updates the slide bar and chips if anything changed
  //
  function pollSlide() {
    ...
    if (titleChanged) {
      ...
      // ── Cross-reference against SLIDE_CONTENT first ───────────────────
      var match = findSlideByTitle(detectedTitle);
      if (match) {
        slideContent = match.content;
      } else {
        // Fallback: scrape visible DOM text for slides not in SLIDE_CONTENT
        var scraped = scrapeSlideContent();
        slideContent = scraped || null;
      }
  ...
  courseSelect.addEventListener("change", function () {
    courseLabel.textContent =
      "course: " + courseSelect.value + " \u00B7 client: " + CLIENT_ID;
  });
  ...
  function appendMessage(role, text) {
    ...
    div.innerHTML =
      '<span class="xact-msg-label">' +
      (role === "user" ? "You" : "XactAI") +
      "</span>" +
      '<div class="xact-bubble">' +
      escapeHtml(text) +
      "</div>";
  ...
  function buildSlideContextBlock() {
    ...
    if (slideContent) {
      ...
      parts.push(
        "(Slide body text was not available — answer based on the title and course knowledge base.)",
      );
    }
  ...
  var SLIDE_CONTENT = {
    1.1: {
      title: "Welcome",
      content:
        "Welcome to Battery Electric Bus Technician Safety and Familiarization - Level 1, a module of the Battery Electric Bus Training program, developed by GILLIG. This module will take up to 50 minutes to complete. Select the Forward arrow to get started.\nFamiliarization with GILLIG's Battery Electric Bus (BEB) | Knowledge to safely engage with electric systems | Overview of bus layout | Introduction to major systems | Identification of High-Voltage (HV) components | Key safety considerations",
    },
    1.2: {
    ...
    4.7: {
      title: "Power Conversion and Distribution",
      content:
        "From the ESS, high-voltage energy is routed through the High-Voltage Junction Box (HVJB), rated up to 750 VDC. This is where power is distributed safely to other systems and provides protection for high-voltage circuits. The DC/DC converter steps HV down to approximately 27 VDC for the low-voltage systems.",
    },
    4.8: {
      title: "Traction and Propulsion",
      content:
        "The bus is driven by a HV traction motor. This motor converts electrical energy into mechanical force to move the wheels. It receives alternating current (AC) from the inverter, which converts high-voltage DC from the ESS into AC for the motor. The traction motor is located at the center chassis behind the rear axle.",
    },
    4.9: {
      title: "Thermal and Climate Control",
      content:
        "HV energy from the ESS powers the bus's thermal and climate control systems, supporting both passenger comfort and the thermal operation of high-voltage components. The HVAC system regulates cabin temperature. The Battery Thermal Management System (BTMS) keeps battery packs within safe operating temperature ranges.",
    },
    "4.10": {
      title: "Safety and Fire Detection",
      content:
        "Working together with the Battery Management System (BMS), fire detection sensors are built into major enclosures including the ESS and continuously monitor for overheating or early signs of thermal runaway. If a fault is detected, the system triggers alerts and can initiate protective shutdowns to prevent escalation.",
    },
    4.11: {
      title: "Rear Powertrain Components",
      content:
        "The powertrain compartment at the rear of the bus is a key location where HV power is distributed and managed. It houses the rear run box, HVJB, DC/DC converter, isolation junction box (IJB), multi string manager (MSM+), and air compressor. These components work together to manage, protect, and distribute HV energy throughout the bus.",
    },
    4.13: {
      title: "Rear Run Box",
      content:
        "The rear run box is located inside the rear powertrain compartment. It contains basic controls and gauges, along with the primary CAN diagnostic connector. The EV system can also be started from the rear run box, which is useful for technicians performing rear-of-bus diagnostics or maintenance.",
    },
    4.14: {
      title: "High-Voltage Junction Box",
      content:
        "The HVJB is the main distribution hub for HV power from the ESS. It routes electricity to the inverter, DC/DC converter, HVAC, and other auxiliaries, acting like a central fuse box for the bus. It is rated up to 750 VDC and includes protection devices for high-voltage circuits.",
    },
    4.15: {
      title: "Isolation Junction Box",
      content:
        "The Isolation Junction Box (IJB) is mounted above the rear run box in the powertrain compartment. It continuously monitors the HV system for potential leakage paths to the chassis ground. If an isolation fault is detected, it triggers an HV exposure fault on the MFD, alerting the operator or technician to a potential safety issue.",
    },
    4.16: {
      title: "DC/DC Converter",
      content:
        "The DC/DC converters step down HV power to approximately 27 volts to supply power to the bus's low-voltage systems. These include the power steering pump, the electronics cooling package, lighting, communications, and control circuits. The DC/DC converter replaces the alternator found on diesel buses.",
    },
    4.17: {
      title: "Multi String Manager +",
      content:
        "The Multi String Manager (MSM+) is mounted in the rear curbside air duct and collects data from each battery pack's monitoring system. It manages charging, temperature, and balance across the ESS, and communicates pack status to the System Control Module (SCM) and other vehicle controllers.",
    },
    4.18: {
      title: "Air Compressor",
      content:
        "The HV air compressor is part of the air system, which operates the brakes, suspension, doors, and driver's seat. Mounted in the powertrain compartment, it is driven by a 650-volt brushless DC motor controlled by the HV inverter. It replaces the engine-driven compressor found on diesel buses.",
    },
    4.19: {
      title: "HV Interlock Loop",
      content:
    ...
    5.2: {
      title: "Electronic Traction Motor and Inverter",
      content:
        "The electric drive system includes a permanent magnet traction motor and an inverter. The motor uses an internal rotor design with a stator, delivering higher power density and lower maintenance than traditional motors. The inverter converts DC power from the ESS into AC power for the motor, and also manages regenerative braking energy recovery.",
    },
    5.3: {
      title: "Regenerative Braking Overview",
      content:
        "During regenerative braking, the traction motor reverses its function and acts as a generator. Instead of wasting energy as heat, it sends electricity back to the ESS. However, regenerative braking has limits — battery charge state and cold temperatures can limit the amount of energy recovered. In those cases, the foundation (friction) brakes handle the additional braking force.",
    },
    6.2: {
      title: "Low Voltage System",
      content:
        "Similar to a diesel bus, the BEB uses a split 12/24-volt LV electrical system to power lighting, doors, communications, and control circuits. Instead of an alternator, the DC/DC converter steps down HV power to supply the LV system. The 24V side powers high-current loads like the hydraulic pump and door systems, while the 12V side handles lighting and electronics.",
    },
    6.3: {
      title: "Controller Area Network",
      content:
        "The BEB uses a Controller Area Network (CAN) communication network to reduce wiring complexity and improve reliability. Instead of running separate wires for every component, digital signals travel on a shared data bus. The protocol used is SAE J1939, the same standard used across commercial vehicles. This allows all controllers to communicate efficiently and enables diagnostic access through a single connector.",
    },
    7.2: {
      title: "Electrical Panels Overview",
      content:
        "The bus has three main electrical panels: the front electrical panel above the driver's area, the above rear door enclosure inside the bus above the exit door, and the rear enclosure electrical panel mounted at the rear bulkhead. Each panel houses LV distribution components, fuses, relays, and control modules for different bus systems.",
    },
    7.3: {
      title: "Front Electrical Panel",
      content:
        "The front electrical panel is located in the electrical component compartment on the ceiling above the driver's area. It houses key low-voltage components that distribute and protect electrical circuits. It includes a diagnostic connector, an interlock override switch, fuses, and relays for front bus systems.",
    },
    7.4: {
      title: "Above Rear Door Enclosure",
      content:
        "The above rear door enclosure is located inside the bus above the passenger exit door. On buses without a rear door, the C1 module is relocated above the front entrance door instead. This enclosure houses LV distribution components and control modules for rear bus systems.",
    },
    7.5: {
      title: "Rear Enclosure Electrical Panel",
      content:
        "The rear enclosure electrical panel is mounted at the top of the rear bulkhead area and is accessed through a locked door. It contains a wide range of components including diagnostic connectors, the main bus controller, data logger, LV distribution components, fuses, and relays. Before servicing, the EV system must be shut down and LV disconnect procedure followed. The EV system must remain powered down for a minimum wait period before opening this panel.",
    },
    8.2: {
      title: "Vehicle System Overview",
      content:
        "While the vehicle systems on the BEB are the same as on a diesel bus, the way they are powered and controlled differs. Instead of being engine-driven, they rely on electrical or electronically controlled components drawing energy from the HV system or through the LV system. Systems include suspension, braking, hydraulic steering, air, HVAC, electronics cooling, and the communication network.",
    },
    8.3: {
      title: "Suspension",
      content:
        "The BEB's suspension system provides vehicle stability and passenger comfort, similar to diesel buses. It uses a 4-bag rear suspension and a 2-bag front suspension. These air springs are managed electronically and support kneeling functions for accessibility. The system is powered by the HV-driven air compressor.",
    },
    8.4: {
      title: "Braking System",
      content:
        "The braking system combines traditional air-actuated brakes with electronic controls and regenerative functions. Primary and secondary air tanks store compressed air for service, emergency, and parking brakes. The HV-driven air compressor maintains system pressure. Regenerative braking supplements friction braking and recovers energy for the ESS.",
    },
    8.5: {
      title: "Hydraulic and Steering System",
      content:
        "The hydraulic system supports power steering. On diesel buses, hydraulic pressure comes from engine-driven pumps. On the BEB, a hydraulic pump driven by the 24V LV system generates the pressure. This means the steering system depends on LV power being available, not engine operation.",
    },
    8.6: {
      title: "Air System",
      content:
        "The BEB's air system supplies compressed air for service brakes, emergency brakes, parking brakes, suspension, and accessibility features such as kneeling. The HV-driven air compressor maintains system pressure. Unlike diesel buses, the air system depends on HV readiness — if HV is not available, the compressor cannot run.",
    },
    8.7: {
      title: "HVAC and Battery Thermal Management",
      content:
    ...
    8.8: {
      title: "Electronics Cooling Package",
      content:
        "The Electronics Cooling Package (ECP) manages heat from HV components other than the batteries, such as the traction motor, inverters, and DC/DC converters. The system includes an air-cooled radiator assembly and cooling fans. It runs off the LV system and operates independently from the HVAC and BTMS.",
    },
    8.9: {
      title: "Communication Network",
      content:
        "A CAN network connects and controls vehicle systems. Digital signals are carried across shared data lines, reducing wiring complexity. The network uses SAE J1939 protocol. All major controllers including the SCM, MSM+, inverter, and body controllers communicate over this network. Diagnostics can be accessed via a single CAN diagnostic connector.",
    },
    9.2: {
      title: "Energy Flow",
      content:
