            ...
            try {
              ...
              if (data.body && typeof data.body === 'string') {
                try {
                  ...
                  if (b.output !== undefined) streamMsg.append(b.output);
                  if (b.session_id) {
      ...
      callAPIStream(body, streamMsg)
        .then(function () {
          if (!streamMsg.getText()) {
            streamMsg.append('No response received. The request may have timed out or the server returned no content. Try again or use the non-streaming endpoint.');
          }
        })
        .catch(function (err) {
          if (!streamMsg.getText()) {
